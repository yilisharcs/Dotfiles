...
**Deprecated in 0.16: `@trunc` can now convert float to integer directly.**
```zig
const i: i32 = @intFromFloat(3.7);  // i = 3
// Preferred in 0.16:
const i: i32 = @trunc(3.7);
```
...
In 0.14+, can change slice length. In 0.15+, can cast single-item pointer to slice.
```zig
const bytes: [*]u8 = @ptrCast(some_ptr);
// Single-item pointer to slice (0.15+):
const val: u32 = 1;
const slice: []const u8 = @ptrCast(&val);
```
...
```
Remove `volatile` from pointer.

### @bitCast
```zig
@bitCast(value: anytype) anytype
```
Reinterpret bit pattern. Return type inferred. Sizes must match.
```zig
...
```
Truncate integer to smaller type. Discards high bits.
```zig
const small: u8 = @truncate(@as(u32, 0x1234));  // 0x34
```

### @intFromBool
```zig
@intFromBool(b: bool) u1
```
...
### @enumFromInt
```zig
@enumFromInt(value: anytype) anytype
```
Convert integer to enum. Return type inferred. **Safety check in safe modes.**

### @errorFromInt
```zig
@errorFromInt(value: anytype) anyerror
```
Convert integer to error.

### @intFromError
```zig
@intFromError(err: anyerror) anytype
```
...
In 0.16, these forward result type, so you can write `const x: f64 = @sqrt(@floatFromInt(N));`.
```zig
@sqrt(x)     // Square root
@sin(x)      // Sine
@cos(x)      // Cosine
...
@log2(x)     // Log base 2
@log10(x)    // Log base 10
@floor(x)    // Round down — can now return integer type
@ceil(x)     // Round up — can now return integer type
@round(x)    // Round to nearest — can now return integer type
@trunc(x)    // Truncate toward zero — can now return integer type
...
```
Copy memory. Slices must not overlap. Source and destination element types must be in-memory coercible.
```zig
...
```
Get type information as tagged union. Fields are **lowercase** (0.14+): `.@"struct"`, `.@"enum"`, `.@"union"`, `.int`, `.pointer`, etc.
```zig
switch (@typeInfo(T)) {
    .@"struct" => |s| {
        for (s.fields) |field| {
...
### @Type (deprecated — use individual builtins)
```zig
@Type(comptime info: std.builtin.Type) type
```
Create type from type info (inverse of `@typeInfo`).
**Deprecated in 0.16.** Replaced by `@Int`, `@Struct`, `@Union`, `@Enum`, `@Pointer`, `@Fn`, `@Tuple`, `@EnumLiteral`. See language.md for full details.

### @EnumLiteral
```zig
@EnumLiteral() type
```
Returns the "enum literal" type. New in 0.16, replaces `@Type(.enum_literal)`.

### @Int
```zig
@Int(comptime signedness: std.builtin.Signedness, comptime bits: u16) type
```
Create integer type. New in 0.16, replaces `@Type(.{ .int = ... })` and `std.meta.Int`.
```zig
...
```
Create tuple type. New in 0.16, replaces `@Type(.{ .@"struct" = ... with is_tuple })` and `std.meta.Tuple`.
```zig
...
```
Create pointer type. New in 0.16, replaces `@Type(.{ .pointer = ... })`.
```zig
const ptr_type = @Pointer(.one, .{ .@"const" = true }, u32, null);  // *const u32
```
...
```
Create function type. New in 0.16, replaces `@Type(.{ .@"fn" = ... })`.
```zig
const fn_type = @Fn(&.{ f64, *const anyopaque }, &.{ .{}, .{ .@"noalias" = true } }, u32, .{ .@"callconv" = .c, .varargs = true });
```
...
```
Create struct type. New in 0.16, replaces `@Type(.{ .@"struct" = ... })`.
...
```
Create union type. New in 0.16, replaces `@Type(.{ .@"union" = ... })`.
...
```
Create enum type. New in 0.16, replaces `@Type(.{ .@"enum" = ... })`.
...
```
Get type of a struct field. New in 0.14.
```zig
const t = @FieldType(MyStruct, "x");  // e.g. u32
```
...
```
Trigger compile error with message.

### @compileLog
```zig
@compileLog(args...) void
```
Print values at compile time (debugging).

### @This
```zig
@This() type
```
Get type of enclosing struct/union/enum.

### @src
```zig
@src() std.builtin.SourceLocation
```
Get source location. In 0.14+, includes `module` field.
```zig
const loc = @src();
// loc.module, loc.file, loc.fn_name, loc.line, loc.column
```
...
@inComptime() bool
```
Check if currently evaluating at compile time.

### @setEvalBranchQuota
```zig
@setEvalBranchQuota(comptime new_quota: u32) void
```
Increase compile-time evaluation iteration limit.
...
In 0.14+, also works on arrays (not just vectors).
```zig
...
**Deprecated in 0.16.** Use `b.addTranslateC(...)` in build.zig instead.
```zig
// BEFORE (deprecated)
const c = @cImport({
    @cDefine("_GNU_SOURCE", {});
...
```
Hint branch likelihood. Must be first statement in branch. New in 0.14, replaces `@setCold`.
```zig
