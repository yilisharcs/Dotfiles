...
in {
    home-manager.sharedModules = [{
        ...
        home.sessionVariables = {
            ZK_NOTEBOOK_DIR = "$HOME/Shared/notebook";
        };
        ...
        programs.zk = enabled {
            settings = {
                ...
                #group."<NAME>".paths = ["<DIR1>" "<DIR2>"];
                #group."<NAME>".note.filename = "{{format-date now}}";
                #group."<NAME>".extra.key = "value";

                # group.chess.note = {
                #         filename = "{{format-date now}}-v-{{slug title}}";
                #         template = "chess.md";
                # };
                group.journal.note = {
                ...
                group.source.note.template = "source.md";
                # MARKDOWN SETTINGS
                format.markdown = {
                ...
                tool = {
                    ...
                    fzf-preview = "${getExe pkgs.bat} -p --color always {-1}";
                };
                # LSP
                #
                ...
                alias = {
                    ...
                    hist = "zk list --format path --delimiter0 --quiet $@ | xargs -t -0 ${getExe pkgs.git} log --patch --";
