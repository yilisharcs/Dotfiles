  ...
  home-manager.sharedModules = [
    ({config, ...}: {
      yaziPrependOpenRules = {
        ...
        archive = {
          mime = "application/{gzip,x-xz,zip,zstd}";
          use = ["edit" "reveal"];
        };
        exec = {
          mime = "application/{pie-executable,executable}";
          use = ["xbin"];
        };
      };
      ...
      programs.yazi = enabled {
        ...
        keymap.mgr.prepend_keymap = map mkKeymap [
          ...
          # Restore trashed files
          {
            on = "u";
            run =
              ''shell 'file=$(${getExe' pkgs.trash-cli "trash-list"} | grep "$(pwd)"''
